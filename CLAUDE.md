...
6. If the sync writes to the shared card table (i.e. affects any filterable field), refresh the filter-options cache at the end of the sync method, inside the same transaction boundary — follow the pattern of the existing sync jobs that already do this. Don't add a live/on-demand recompute path instead; the cache table is the single source of truth for the filters endpoint.
